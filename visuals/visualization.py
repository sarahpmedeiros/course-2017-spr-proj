...
class visualization(dml.Algorithm):
        ...
        def setBounds(l,h):
            visualization.lowBound = l
            visualization.highBound = h

        @staticmethod
        def execute(trial=False):

                print(visualization.lowBound)
                print(visualization.highBound)
                ...
                plt.plot(X,Y,'ro')
                plt.plot([0,max(X)],[B0,B0+max(X)*B1])
                plt.show()
